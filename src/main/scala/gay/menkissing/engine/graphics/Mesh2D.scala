...
final case class GayM(vertices: Array[Float], indices: Array[Int]) derives ReadWriter
...
object Mesh2D {
  ...
  /**
    * Load a precompiled mesh into memory.
    * 
    * A Precompiled mesh (GayM) is a mess
    *
    * @return
    */
  def loadGayM(source: InputStream): Mesh2D = {
    val mesh = readBinary[GayM](source)
    val buf = memAllocFloat(mesh.vertices.length)
    buf.put(0, mesh.vertices)
    val idxBuf = memAllocInt(mesh.indices.length)
    idxBuf.put(0, mesh.indices)
    Mesh2D.textured(buf, idxBuf, PrimitiveType.Triangles)
  }
